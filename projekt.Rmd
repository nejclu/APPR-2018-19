...
kable(head(tidy_economy), align="ccc")
``` 
...
kable(head(tidy_family), align="ccc")
``` 
...
kable(head(tidy_life.expectancy), align="ccc")
```   
...
kable(head(tidy_freedom), align="ccc")
```   
...
kable(head(tidy_trust), align="ccc")
```     
...
kable(head(tidy_generosity), align="ccc")
```     
  
7. **Dystopia + preostanek** - Podatki o pomenu dodatne komponente pri stopnji sreče ljudi
  - `Država` - spremenljivka: ime države (besedilo)
  - `Leto` - spremenljivka: leto, katerega so bile opravljene meritve (celo število)
  - `Dystopia + preostanek` - meritev: v kolikšni meri rezultat države »Dystopia« in ocena napake prejšnjih meritev vpliva na izračun stopnje sreče ljudi (število)
```{r tidy_residual, echo=FALSE, message=FALSE}
kable(head(tidy_residual), align="ccc")
```     
...
```{r tidy_preb, echo=FALSE, message=FALSE}
kable(head(tidy_preb), align="cc", col.names=c("Drzava", "Prebivalstvo"))
```     

8. **Prirast prebivalstva** - Podatki o prirastu prebivalcev po državah (2016/2017)
  - `Država` - spremenljivka: ime države (besedilo)
  - `Prirast` - spremenljivka: prirast prebivalstva v državi med letoma 2016 in 2017 (izraženo v procentih) (število)
```{r tidy_prir, echo=FALSE, message=FALSE}
kable(head(tidy_prir), align="cc")
```     
